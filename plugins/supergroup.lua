...
local function lock_group_badw(msg, data, target)
  ...
  if group_badw_lock == 'yes' then
    return 'Badword typing is already locked'
  else
    data[tostring(target)]['settings']['lock_badw'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Badword typing has been locked'
  end
end
...
local function unlock_group_badw(msg, data, target)
  ...
  if group_badw_lock == 'no' then
    return 'Badword typing is not locked'
  else
    data[tostring(target)]['settings']['lock_badw'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Badword typing has been unlocked'
  end
end
-- Badword typing fix it.

-- supergroups lock_english
local function lock_group_eng(msg, data, target)
  ...
  if group_eng_lock == 'yes' then
    return 'English typing is already locked'
  else
    data[tostring(target)]['settings']['lock_eng'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'English typing has been locked'
  end
end
...
local function unlock_group_eng(msg, data, target)
  ...
  if group_eng_lock == 'no' then
    return 'English typing is not locked'
  else
    data[tostring(target)]['settings']['lock_eng'] = 'no'
    save_data(_config.moderation.data, data)
    return 'English typing has been unlocked'
  end
end
-- supergroups lock_english  fix it
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'farsi/Persian is already unlocked'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'farsi/Persian has been unlocked'
  end
end
...
local text = reply_msg(msg.id, '|SuperGroup settings:\n|Lock spam : '..settings.lock_spam..'\n|Lock flood : '..settings.flood..'\n|Flood sensitivity : '..NUM_MSG_MAX..'\n|Lock Member : '..settings.lock_member..'\n|Lock Badwords : '..settings.lock_badw..'\n|Lock adds : '..settings.lock_link..'\n|Lock emoji : '..settings.lock_emoji..'\n|Lock sticker : '..settings.lock_sticker..'\n|Lock farsi 🗣 : '..settings.lock_arabic..'\n|Lock English 🗣 : '..settings.lock_eng..'\n|Lock RTL : '..settings.lock_rtl..'\n|Public : '..settings.public..'\n|Strict settings : '..settings.strict, ok_cb, false)
  return text
end
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return reply_msg(msg.id, 'Chat ID: '..msg.to.id .. '\nUser ID: '..msg.from.id, ok_cb, false)
			end
		end
		...
		if matches[1] == 'lock' and is_momod(msg) then
			...
			if matches[2] == 'badw' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] Badword typing ")
		...
		if matches[1] == 'setflood' then
			...
			if tonumber(matches[2]) < 0 or tonumber(matches[2]) > 85 then
				return "Wrong number,range is [0-85]"
			end
