local function save_filter(msg, name, value)
  ...
  if msg.to.type == 'channel' then
    hash = 'channel:'..msg.to.id..':filters'
  end
  if hash then
...
local function get_filter_hash(msg)
  if msg.to.type == 'channel' then
    return 'channel:'..msg.to.id..':filters'
  end
end 
...
local function get_filter(msg, var_name)
  ...
  if hash then
    ...
    elseif value == 'kick' then
      send_large_msg('channel#id'..msg.to.id, "به دلیل عدم رعایت قوانین گفتاری از ادامه ی گفتوگو محروم میشوید")
